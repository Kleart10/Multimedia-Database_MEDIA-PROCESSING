...
/*
 * Given a flat list of media items, build one section per type
 * and insert cards into each section. This is used when the
 * "All" filter is active to visually group images, audio and videos.
 */
function renderGallerySections(items) {
    ...
    types.forEach(type => {
        ...
        section.innerHTML = `
            ...
            <div class="media-grid" id="${type}-grid"></div>
        `;
        wrapper.appendChild(section);

        const grid = section.querySelector('.media-grid');
        const filtered = items.filter(i => i.media_type === type);
        if (filtered.length > 0) {
            filtered.forEach((m, i) => {
                grid.insertAdjacentHTML('beforeend', createMediaCardAnimated(m, i));
            });
            ...
            grid.innerHTML = `<div class="section-empty">No ${type}s found.</div>`;
        }
    });
}
...
function switchView(pageName) {
    ...
    // show/hide floating upload button
    const uploadBtn = document.querySelector('.upload-shortcut');
    if (uploadBtn) uploadBtn.style.display = pageName === 'home' ? 'block' : 'none';
...
async function loadRecentMedia() {
    const wrapper = document.getElementById('media-wrapper');
    if (!wrapper) {
        console.error('[MMDB] media-wrapper container not found!');
        return;
    }

    // remove any leftover sentinel from previous view
    const existingSentinel = document.getElementById('scroll-sentinel');
    ...
    try {
        ...
        console.log('[MMDB] loadRecentMedia - Fetching items for', state.currentFilter);

        const res = await fetch(url);
        const data = await res.json();

        console.log('[MMDB] loadRecentMedia - Got', data.items?.length || 0, 'items');

        if (data.items && data.items.length > 0) {
            state.galleryItems = data.items;
            state.galleryTotal = data.items.length;

            // Render items grouped or single section depending on filter
            if (state.currentFilter === 'all') {
                renderGallerySections(data.items);
                // we won't use infinite scroll when grouping
            } else {
                ...
                section.className = 'media-section';
                section.innerHTML = `
                    <h2 class="section-title">${state.currentFilter.charAt(0).toUpperCase() + state.currentFilter.slice(1)}s</h2>
                    <div class="media-grid" id="media-grid"></div>
                `;
                wrapper.appendChild(section);
                const container = section.querySelector('#media-grid');
                data.items.forEach((m, i) => {
                    const card = createMediaCardAnimated(m, i);
                    container.insertAdjacentHTML('beforeend', card);
                });
                ...
                // keep infinite scroll for filtered view
                initInfiniteScroll();
            }
            ...
            // Update scroll indicator only if not grouped
            if (state.currentFilter !== 'all') updateScrollIndicator();
        } else {
            wrapper.innerHTML = '';
            const emptyState = document.getElementById('empty-state');
        ...
        if (wrapper) {
            wrapper.innerHTML = '<div class="empty-state"><h3>Failed to load media</h3><p>Please check your connection</p></div>';
        }
    }
...
function showSearchLoading() { const results = document.getElementById('search-results'); results.style.display = 'block'; const wrapper = document.getElementById('results-wrapper'); if (wrapper) wrapper.innerHTML = '<div class="loading-spinner"></div>'; document.getElementById('results-count').textContent = 'Searching...'; document.getElementById('query-time').textContent = ''; }
...
function displayResults(data, elapsedTime = null) {
    ...
    if (!data.results || data.results.length === 0) {
        if (wrapper) wrapper.innerHTML = '<div class="empty-state"><h3>No results found</h3></div>';
        return;
    }
    ...
    // group results by media_type to make sections
    const groups = { image: [], audio: [], video: [] };
    data.results.forEach(r => {
        if (r.media_type && groups[r.media_type]) groups[r.media_type].push(r);
        else groups.image.push(r); // fallback
    });
    ...
    ['image', 'audio', 'video'].forEach(type => {
        ...
        if (list && list.length) {
            wrapperHtml += `<div class="results-section">
    ...
    if (wrapper) wrapper.innerHTML = wrapperHtml;

    // attach click events after inserting
    if (wrapper) {
        wrapper.querySelectorAll('.result-card').forEach(card => {
            card.addEventListener('click', () => openMediaModal(card.dataset.id));
        });
    }
}
